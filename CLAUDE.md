...
- `experiments_downstream/run_all_downstream.py` now includes a
  `generate-aug` subcommand that sequentially generates all augmented
  test datasets (`aug_test.jsonl` and `aug_test_<op>.jsonl`) across
  tasks and operators. Each script parallelises rows internally.
  Supports `--force`, `--dry-run`, and `--cumulative/--no-cumulative`.
