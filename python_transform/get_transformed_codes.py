from transform import (
    TRANSFORMATION_MAP,
    convert_python2_to_python3,
    convert_source_to_ast_module,
)
...
def get_transformed_codes(source_code: str) -> tuple[str, str, str]:
    ...
    def unparse_ast(
        transformer: ast.NodeTransformer | None = None, original_code: str = ""
    ) -> Maybe[str]:
        maybe_module: Maybe[ast.Module] = convert_source_to_ast_module(source_code)
        ...
        try:
            if transformer is None:
                ...
                transformed_code = (
                    maybe_module.map(transformer.visit).map(ast.unparse).value_or(None)
                )
                return (
                    Some(transformed_code)
                    if (original_code is not None and transformed_code != original_code)
                    else Nothing
                )
    ...
    transformed_results: List[Maybe[str]] = [
        unparse_ast(TRANSFORMATION_MAP[k](), original_code)
        for k in TRANSFORMATION_MAP.keys()
    ]
    ...
    return tuple(
        result.value_or(None) for result in transformed_results if result != Nothing
    )
