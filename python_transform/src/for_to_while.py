...
class ForToWhileTransformer(ast.NodeTransformer):
    ...
    def _transform_range_for(self, node: ast.For) -> ast.AST:
        ...
        assign_i = ast.Assign(targets=[node.target], value=start)
        ...
        condition = ast.Compare(
            left=node.target,
            ops=(
                [ast.Lt()] if self._is_Lt_equivalent_in_while_loop(step) else [ast.Gt()]
            ),
            comparators=[stop],
        )
        ...
        while_node = ast.While(test=condition, body=new_body, orelse=node.orelse)
    ...
    def _is_Lt_equivalent_in_while_loop(self, step):
        ...
        if isinstance(step, ast.UnaryOp):
            ...
            res = 1
            # if negative sign
            if not isinstance(step.op, ast.UAdd):
                res *= -1
    ...
    def _transform_iterable_for(self, node: ast.For) -> ast.AST:
        ...
        iter_call = ast.Call(
            func=ast.Name(id="iter", ctx=ast.Load()), args=[node.iter], keywords=[]
        )

        assign_iter = ast.Assign(targets=[temp_iter_name], value=iter_call)
        ...
        try_block = ast.Try(
            body=[assign_next], handlers=[except_handler], orelse=[], finalbody=[]
        )
