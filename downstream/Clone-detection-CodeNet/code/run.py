...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    result = {"eval_loss": float(perplexity), "eval_map": float(np.mean(MAP))}

    return result


def get_n_retrieval(eval_dataset):
    """dynamically check the number of retrieval for each query, i.e. R in mAP@R"""
    label_examples: dict[str, int] = {}
    for e in eval_dataset.examples:
        if e.label not in label_examples:
            label_examples[e.label] = 0
        label_examples[e.label] += 1
...
def test(args, model, tokenizer):
    ...
    n_retrieval = get_n_retrieval(eval_dataset)
    ...
    logger.info("  Num examples = %d", len(eval_dataset))
    logger.info("  Batch size = %d", args.eval_batch_size)
    logger.info("  Num retrieval = %d for each query", n_retrieval)
    eval_loss = 0.0
    nb_eval_steps = 0
    model.eval()
    vecs = []
    ...
    with open(prediction_file, "w") as f:
        for index, sort_id in zip(indexs, sort_ids):
            ...
            js["index"] = index
            js["answers"] = []
            for idx in sort_id[:n_retrieval]:
