...
def postprocess(original: pd.DataFrame, transformed_path: Path, rule_id: int) -> pd.DataFrame:
    """Process SPAT output
...
def spat(original: pd.DataFrame, spat_jar: Path, rule_ids: list[int],
         lib_path: Path) -> pd.DataFrame:
    ...
    for rule_id in rule_ids:
        ...
        subprocess.run(
            ["java", "-jar", spat_jar, str(rule_id), original_path,
