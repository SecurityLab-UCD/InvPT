...
from code_transforms import LocalVariableRenamer, FunctionDefinitionReorder, ReverseIfElser, StatementOrderRearrangement, WhileToForTransformer, ForToWhileTransformer, OpAssignment2EqualAssignment, AugType
import ast
import sys
import os
import json
...
def convert_python2_to_python3(source_code: str, filename: str = "random.py") -> str:
    ...
    try:        
        subprocess.run(["2to3", filename, "-w"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def transform_and_write(code_transformer, source_file: str, target_directory: str) -> None:
    """
    ...
    target_filename = os.path.join(target_directory, source_basename.replace('.jsonl', f'_transformed_{code_transformer.method}.jsonl'))

    with jsonlines.open(source_file) as reader, jsonlines.open(target_filename, mode='w') as writer:
        i = 0
        for obj in reader:                        
            
            try:
                transformed_code = apply_AST_transform_and_write(obj["code"], code_transformer)
            except RuntimeError: 
...
def main(argv=None):
    ...
    print('-------- Selected Transforming Method: ', code_transformer.method, ' -------- \n')    
    ...
    if os.path.isdir(args.root):
        ...
        transform_and_write(code_transformer=code_transformer, source_file=args.root, target_directory=args.target)
