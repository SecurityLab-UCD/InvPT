...
from cpp_transforms.transformations.cursor_util import is_if


def if_else_reverser(root_node: Cursor, file_code: str) -> str:
    return if_else_reverse(root_node, "example.cpp", file_code, (0, len(file_code)))


def if_else_reverse(
    root_node: Cursor, source_file: str, source_code: str, start_end: tuple[int, int]
) -> str:
    ...
    while len(to_visit) > 0:
        ...
        if is_if(curr_visit) and source_file in str(curr_visit.location):
            ...
            expr_one = if_else_reverse(
                children[1],
                source_file,
                source_code,
                get_node_char_positions(children[1], source_code),
            )
            expr_two = ""
            if len(children) > 2:
                expr_two = if_else_reverse(
                    children[2],
                    source_file,
                    source_code,
                    get_node_char_positions(children[2], source_code),
                )
            # Get changed code
    ...
    for change_node in change_nodes:
        ...
        i += 1
        file_code = (
            file_code[:offset] + hidden_name + file_code[offset + len(hidden_name) :]
        )
