...
from cpp_transforms.transformations.cursor_util import is_for


def for_while_reverser(root_node: Cursor, file_code: str) -> str:
    return for_while_reverse(root_node, "example.cpp", file_code, (0, len(file_code)))


def for_while_reverse(
    root_node: Cursor,
    source_file: str,
    source_code: str,
    ...
    while len(to_visit) > 0:
        ...
        if is_for(curr_visit) and source_file in str(curr_visit.location):
            ...
            body = for_while_reverse(
                children[3],
                source_file,
                source_code,
                get_node_char_positions(children[3], source_code),
            )
            changed_code = "%s\nwhile(%s) {%s\n%s;}" % (
                decl_stmt,
                condition,
